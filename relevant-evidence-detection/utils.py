...
class DatasetIterator_negative_Evidence(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_evidence_neg > 0:
            current_negative_images = current.get('negative_images', [])
            current_negative_texts = current.get('negative_texts', [])
            if not isinstance(current_negative_images, list):
                current_negative_images = []
            if not isinstance(current_negative_texts, list):
                current_negative_texts = []
            
            total_items = 2*self.use_evidence + 2*self.use_evidence_neg
            available_X_img_neg = len(current_negative_images[:self.use_evidence_neg])
            available_X_txt_neg = len(current_negative_texts[:self.use_evidence_neg])
            ...
            if negatives_to_add > 0:
                max_images = max(0, len(current_negative_images) - self.use_evidence_neg)
                max_texts = max(0, len(current_negative_texts) - self.use_evidence_neg)
                ...
                if how_many_neg_imgs + how_many_neg_txts < negatives_to_add:              
                    for _ in range(negatives_to_add - how_many_neg_imgs - how_many_neg_txts):
                        ...
                        current_match_index = str(current.get('match_index', ''))
                        current_split = current_match_index.split("_")[0] if "_" in current_match_index else ""
                        avoid_id_pattern = current_match_index + "_" if current_match_index else ""
                        
                        if rand_modality == 0:
                            avoid_columns = [col for col in self.X_visual_features.columns if avoid_id_pattern and col.startswith(avoid_id_pattern)]
                            valid_columns = [col for col in self.X_visual_features.columns if col not in avoid_columns and (not current_split or col.startswith(current_split))]
                            if len(valid_columns) == 0:
                                valid_columns = [col for col in self.X_visual_features.columns if col not in avoid_columns]
                            selected_item = random.choice(valid_columns)
                            extra_random_imgs.append(selected_item)
                            
                        else:
                            avoid_columns = [col for col in self.X_textual_features.columns if avoid_id_pattern and col.startswith(avoid_id_pattern)]
                            valid_columns = [col for col in self.X_textual_features.columns if col not in avoid_columns and (not current_split or col.startswith(current_split))]
                            if len(valid_columns) == 0:
                                valid_columns = [col for col in self.X_textual_features.columns if col not in avoid_columns]
                            selected_item = random.choice(valid_columns)                      
                            extra_random_txts.append(selected_item)
                                                                        
                                
            X_img_neg_items = current_negative_images[:self.use_evidence_neg + how_many_neg_imgs] + extra_random_imgs            
            X_img_neg = self.X_visual_features[X_img_neg_items].T.values
        
            X_txt_neg_items = current_negative_texts[:self.use_evidence_neg + how_many_neg_txts] + extra_random_txts            
...
def sensitivity_per_class(y_true, y_pred, C):
    ...
    if C == 2:
        y_true = np.ones(y_true.shape[0], dtype=y_true.dtype)

    return round(float((y_pred == y_true).mean()), 4)
...
def eval_dataset(model, verite_data_generator, fusion_method, use_evidence, fuse_evidence, device, zero_pad=False, label_map={'true': 0, 'miscaptioned': 1, 'out-of-context': 2}, cur_epoch=-3):
...
def load_verite(data_path, encoder, encoder_version, label_map={'true': 0, 'miscaptioned': 1, 'out-of-context': 2}):
    ...
    verite_test.falsified = verite_test.falsified.replace(label_map)
...
def early_stop(has_not_improved_for, model, optimizer, history, 
        current_epoch, PATH, metrics_list, scheduler=None):
    # Get the history as a DataFrame
    history_df = pd.DataFrame(history)
    
    # Find the best model based on metrics
    best_model = choose_best_model(history_df, metrics=metrics_list)
    best_index = best_model.index[0]
    best_epoch = best_model.iloc[0].get('epoch', best_index)  # Get epoch number if available
    
    # Determine if the current epoch is the best so far
    is_current_best = (best_index == len(history) - 1)  # Is the latest entry the best?
    
    if not os.path.isdir(PATH.split('/')[0]):
        os.mkdir(PATH.split('/')[0])
    
    print("Accuracy is: {}".format(history[len(history)-1]['Accuracy']))
    current_results = history[-1]
    ...
    # Check for 90% or higher accuracy - special milestone save
    if accuracy >= 0.9:
        milestone_str = f"{accuracy:.4f}".replace(".", "_")
        milestone_path = f"{os.path.splitext(PATH)[0]}_MILESTONE_{milestone_str}_epoch_{current_epoch}{os.path.splitext(PATH)[1]}"
        
        print(f"🎉 MILESTONE REACHED! Saving 90%+ accuracy checkpoint ({accuracy:.4f}) at epoch {current_epoch}! 🎉\n")
        torch.save(checkpoint, milestone_path)
    
    # Regular high accuracy save (80%+)
    if accuracy >= 0.88:
        accuracy_str = f"{accuracy:.4f}".replace(".", "_")
        high_acc_path = f"{os.path.splitext(PATH)[0]}_nacc_{accuracy_str}_epoch_{current_epoch}{os.path.splitext(PATH)[1]}"
        
        print(f"Saving high accuracy checkpoint ({accuracy:.4f}) at epoch {current_epoch}!\n")
        torch.save(checkpoint, high_acc_path)
    
    # Check if current epoch is the best so far
    if is_current_best:
        print(f"Checkpoint! Current epoch {current_epoch} is the best so far.\n")
        torch.save(checkpoint, PATH)
        has_not_improved_for = 0
    else:
        print(f"DID NOT CHECKPOINT! Best was epoch {best_epoch}, current is {current_epoch}\n")
...
def load_ranked(data_name, encoder, choose_encoder_version, data_path, label_map={'true': 0, 'miscaptioned': 1, 'out-of-context': 2}):
    
    encoder_version = choose_encoder_version.replace('-', '').replace('/', '')
    
    verite_test = pd.read_csv(data_path + "{}_ranked_evidence_".format(data_name) + encoder.lower() + "_" + encoder_version +  ".csv", index_col=0)
    ...
    verite_text_embeddings = np.load(data_path + "{}_".format(data_name) + encoder.lower() + "_text_embeddings_" + encoder_version + ".npy").astype('float32')
    verite_image_embeddings = np.load(data_path + "{}_".format(data_name) + encoder.lower() +"_image_embeddings_" + encoder_version + ".npy").astype('float32')
    verite_image_embeddings = pd.DataFrame(verite_image_embeddings, index=verite_test.id.values).T
    verite_text_embeddings = pd.DataFrame(verite_text_embeddings, index=verite_test.id.values).T
